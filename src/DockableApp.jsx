...
export const DockableApp = ({
  ...
  const adjustZoom = useCallback((component, delta) => {
    setPanelZoom((prev) => {
      ...
      const newIdx = Math.max(
        ...
        Math.min(
          ZOOM_STEPS.length - 1,
          (currentIdx >= 0 ? currentIdx : 3) + delta,
        ),
      );
  ...
  const handleSpotClick = useCallback(
    (spot) => {
      ...
      if (spot.lat && spot.lon) {
        ...
        const path = (dxClusterData.paths || []).find(
          (p) => p.dxCall === spot.call,
        );
      ...
      >
        ...
        {handleToggleDxLock && (
          <button
            ...
            title={
              dxLocked
                ? "Unlock DX position (allow map clicks)"
                : "Lock DX position (prevent map clicks)"
            }
            style={{
              background: dxLocked
                ? "var(--accent-amber)"
              ...
              border:
                "1px solid " +
                (dxLocked ? "var(--accent-amber)" : "var(--border-color)"),
              borderRadius: "4px",
              padding: "2px 6px",
              fontSize: "10px",
  ...
  const factory = useCallback(
    (node) => {
      ...
      switch (component) {
        ...
        case "solar-image":
          content = (
            <SolarPanel solarIndices={solarIndices} forcedMode="image" />
          );
          break;

        case "solar-indices":
          content = (
            <SolarPanel solarIndices={solarIndices} forcedMode="indices" />
          );
          break;

        case "solar-xray":
          content = (
            <SolarPanel solarIndices={solarIndices} forcedMode="xray" />
          );
          break;

        case "lunar":
          content = (
            <SolarPanel solarIndices={solarIndices} forcedMode="lunar" />
          );
          break;
        ...
        case "dxpeditions":
          content = (
            <DXpeditionPanel
              data={dxpeditions.data}
              loading={dxpeditions.loading}
            />
          );
          break;
        ...
        case "sota":
          content = (
            <SOTAPanel
              ...
              showOnMap={mapLayers.showSOTA}
              onToggleMap={toggleSOTA}
              onSpotClick={handleSpotClick}
            />
          );
          break;
        ...
        default:
          content = (
            <div
              style={{ padding: "20px", color: "#ff6b6b", textAlign: "center" }}
            >
  ...
  return (
    ...
    >
      ...
      {/* Dockable Layout */}
      <div
        style={{ flex: 1, position: "relative", padding: "8px", minHeight: 0 }}
      >
