...
const PSKReporterPanel = ({
  ...
  const filterReports = (reports) => {
    return reports.filter((r) => {
      if (filters?.bands?.length && !filters.bands.includes(r.band))
        return false;
      if (filters?.grids?.length) {
        const grid = activeTab === "tx" ? r.receiverGrid : r.senderGrid;
        if (!grid) return false;
        if (!filters.grids.includes(grid.substring(0, 2).toUpperCase()))
          return false;
      }
      if (filters?.modes?.length && !filters.modes.includes(r.mode))
        return false;
      return true;
    });
  };
  ...
  const filteredTx = useMemo(
    () => filterReports(txReports),
    [txReports, filters, activeTab],
  );
  const filteredRx = useMemo(
    () => filterReports(rxReports),
    [rxReports, filters, activeTab],
  );
  const filteredReports = activeTab === "tx" ? filteredTx : filteredRx;
  const pskFilterCount = [
    filters?.bands?.length,
    filters?.grids?.length,
    filters?.modes?.length,
  ].filter(Boolean).length;

  const getFreqColor = (freqMHz) =>
    !freqMHz ? "var(--text-muted)" : getBandColor(parseFloat(freqMHz));
  const formatAge = (m) =>
    m < 1
  ...
  const wsjtxFilterOptions = useMemo(() => {
    const bands = [
      ...new Set(wsjtxDecodes.map((d) => d.band).filter(Boolean)),
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
            ...
            <>
              ...
              <select
                ...
                style={{
                  ...
                  color:
                    wsjtxFilter !== "all" ? "#a78bfa" : "var(--text-primary)",
          ...
          {handleMapToggle && (
            <button
              ...
              style={iconBtn(
                isMapOn,
                panelMode === "psk" ? "#4488ff" : "#a78bfa",
              )}
              title={
                isMapOn
                  ? t("pskReporterPanel.map.hide")
                  : t("pskReporterPanel.map.show")
              }
            >
      ...
      >
        {panelMode === "psk" ? (
          <>
            ...
            >
              ▲{" "}
              {t("pskReporterPanel.tabs.heard", {
                count: pskFilterCount > 0 ? filteredTx.length : txCount,
              })}
            </button>
            <button
            ...
            >
              ▼{" "}
              {t("pskReporterPanel.tabs.hearing", {
                count: pskFilterCount > 0 ? filteredRx.length : rxCount,
              })}
            </button>
          </>
        ) : (
          <>
            ...
            >
              {t("pskReporterPanel.wsjtx.decodes", {
                count: filteredDecodes.length,
              })}
            </button>
            <button
          ...
          <>
            {!callsign || callsign === "N0CALL" ? (
              ...
              filteredReports.slice(0, 25).map((report, i) => {
                const freqMHz =
                  report.freqMHz ||
                  (report.freq ? (report.freq / 1000000).toFixed(3) : "?");
                ...
                return (
                  ...
                  >
                    <span
                      style={{ color, fontWeight: "600", fontSize: "10px" }}
                    >
                      {freqMHz}
                    </span>
                    <span
            ...
            wsjtxDecodes.length === 0 ? (
              ...
              >
                <div style={{ fontSize: "12px" }}>
                  {t("pskReporterPanel.wsjtx.waiting")}
                </div>
                {wsjtxRelayEnabled ? (
                  wsjtxRelayConnected ? (
                    <div
                  ...
                  <div
                    style={{ fontSize: "10px", opacity: 0.6, lineHeight: 1.5 }}
                  >
                    {t(
                      "                                                pskReporterPanel.wsjtx.udpPath",
                    )}
                    <br />
                    {t("pskReporterPanel.wsjtx.udpAddress", {
                      port: wsjtxPort || 2237,
                    })}
                  </div>
                )}
                  ...
                  filteredDecodes.map((d, i) => (
                    ...
                    >
                      ...
                      <span
                        style={{
                          color: d.band
                            ? getBandColor(d.dialFrequency / 1000000)
                            : "var(--text-muted)",
                          minWidth: "32px",
                          textAlign: "right",
                          fontSize: "10px",
                  ...
                  [...wsjtxQsos].reverse().map((q, i) => (
                    ...
                    >
                      <span
                        style={{
                          color: q.band
                            ? getBandColor(q.frequency / 1000000)
                            : "var(--accent-green)",
                          fontWeight: "600",
                          minWidth: "65px",
                        }}
                      ...
                      {q.dxGrid && (
                        <span style={{ color: "#a78bfa", fontSize: "10px" }}>
                          {q.dxGrid}
                        </span>
                      )}
                    </div>
      ...
      {panelMode === "wsjtx" && activeClients.length > 0 && (
        ...
        >
          <span>
            {activeClients
              .map(([id, c]) => `${id}${c.version ? ` v${c.version}` : ""}`)
              .join(", ")}
          </span>
